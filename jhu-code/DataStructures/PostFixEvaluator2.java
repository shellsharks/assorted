...
public class PostFixEvaluator2 {
	...
	String stack[]; // Holds the stack
	int stackpointer; // Holds the stack pointer
	
	private void push(String pushed) {
		stack[stackpointer++] = pushed; // Push TEMPn onto the stack
	}
	...
	public static void main(String[] args) throws IOException {
		...
		PostFixEvaluator2 machine = new PostFixEvaluator2();
			...
			*/
			try {
				for (int j = 0; j < 9999999; j++) {
					...
					machine.stackpointer = 0; // Keeps track of the top of the stack
					...
					*/
					try {
						...
						machine.stack = new String[line.length()]; // Initializes stack. Stack size is set to length of expression. 
						...
						for (int i = 0; i < length; i++) { // Iterates through entire postfix expression character by character
							...
							*/
							...
							if (isOperator) {								
								...
								stage[0] = machine.pop(); // Pop the operand stack --> Sets the second value in expression
								...
								machine.push(TEMP + Integer.toString(tempn++));
								output += INS[6] + " " + machine.peek() + ";";
							}
